...
kbar=2*pi;
I=0.7;%current
A0=pi;%initial area at constricted region
Te0=1;%Temperature
...
E_replace=zeros(M,N);%to store numerical solution for electric field
...
E_replace(1,1)=-(u_e(1,2)^2-u_e(1,N)^2)/(4*h)-Te(1,1)/(2*h)*(log(density(1,2))-log(density(1,N)));
E_replace(1,2:N-1)=-(u_e(1,3:N).^2-u_e(1,1:N-2).^2)/(4*h)-Te(1,2:N-1)/(2*h).*(log(density(1,3:N))-log(density(1,1:N-2)));
E_replace(1,N)=-(u_e(1,1)^2-u_e(1,N-1)^2)/(4*h)-Te(1,N)/(2*h)*(log(density(1,1))-log(density(1,N-1)));
...
for n=1:M-1
    ...
    u_ion(n+1,1)=u_ion(n,1)+dt/mi*(E_replace(n,1))-dt/(4*h)*(u_ion(n,2)^2-u_ion(n,N)^2);
    u_ion(n+1,2:N-1)=u_ion(n,2:N-1)+dt/mi*(E_replace(n,2:N-1))-dt/(4*h)*(u_ion(n,3:N).^2-u_ion(n,1:N-2).^2);
    u_ion(n+1,N)=u_ion(n,N)+dt/mi*(E_replace(n,N))-dt/(4*h)*(u_ion(n,1)^2-u_ion(n,N-1)^2);
    u_e(n+1,:)=-I./(density(n+1,:).*A(n+1,:));
    E_replace(n+1,1)=-(u_e(n+1,2)^2-u_e(n+1,N)^2)/(4*h)-Te(n+1,1)/(2*h)*(log(density(n+1,2))-log(density(n+1,N)));
    E_replace(n+1,2:N-1)=-(u_e(n+1,3:N).^2-u_e(n+1,1:N-2).^2)/(4*h)-Te(n+1,2:N-1)/(2*h).*(log(density(n+1,3:N))-log(density(n+1,1:N-2)));
    E_replace(n+1,N)=-(u_e(n+1,1)^2-u_e(n+1,N-1)^2)/(4*h)-Te(n+1,N)/(2*h)*(log(density(n+1,1))-log(density(n+1,N-1)));
end
